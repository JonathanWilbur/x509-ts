...
class RelativeDistinguishedName {
    ...
    public toString () : string {
        return this.value.map((atav : AttributeTypeAndValue) : string => {
    ...
    public static fromElement (value : DERElement) : RelativeDistinguishedName {
        ...
        return new RelativeDistinguishedName(
            relativeDistinguishedNameElements.map((element : DERElement) : AttributeTypeAndValue => {
                return AttributeTypeAndValue.fromElement(element);
            })
        );
    }
    ...
    public toElement () : DERElement {
        ...
        relativeDistinguishedNameElement.set = this.value.map((atav : AttributeTypeAndValue) : DERElement => {
