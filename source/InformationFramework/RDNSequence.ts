...
class RDNSequence {
    ...
    public toString () : string {
        return this.value.map((rdn : RelativeDistinguishedName) : string => {
            return rdn.toString();
        }).join(",");
    }
    ...
    public static fromElement (value : DERElement) : RDNSequence {
        ...
        return new RDNSequence(
            rdnSequenceElements.map((element : DERElement) : RelativeDistinguishedName => {
                return RelativeDistinguishedName.fromElement(element);
            })
        );
    }
    ...
    public toElement () : DERElement {
        ...
        rdnSequenceElement.set = this.value.map((rdn : RelativeDistinguishedName) : DERElement => {
